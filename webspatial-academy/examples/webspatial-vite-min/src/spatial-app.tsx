...
// PICO WebSpatial Academy change: the upstream fixture renders <Model> everywhere, which in a
// plain browser falls back to an unknown <model> tag and React logs a console error. Render it
// only inside a WebSpatial runtime (the UA carries `WebSpatial/`, the same check the SDK makes),
// and give PICO a GLB: the upstream cone is USDZ, which is the visionOS format.
const inSpatialRuntime =
  typeof navigator !== 'undefined' && /WebSpatial\//i.test(navigator.userAgent)
const isPico =
  typeof navigator !== 'undefined' && /PicoWebApp\/|PicoBrowser/i.test(navigator.userAgent)
const modelSrc = isPico ? '/modelasset/avocado.glb' : '/modelasset/cone.usdz'
  ...
  return (
    ...
    >
      ...
      {inSpatialRuntime ? (
        ...
        >
          Flat browser: the 3D model appears here once the page runs as an installed PICO web app.
        </div>
      )}
    </main>
  )
