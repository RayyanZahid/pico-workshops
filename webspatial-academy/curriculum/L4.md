...
The manifest requests the game window's size, because it's the start scene:
...
**Observed on PICO OS 6.0.0 (2026-09-24):** the installed web app opened at **1280×720** regardless of `xr_main_scene.default_size`, and second windows opened at the same size in the main window's slot. So **design lab pages to fit 1280×720 without scrolling.** p4 now uses a 560 px playfield with the controls in a right-hand column; in the taller layout the ship and Fire button fell off the bottom edge. Keep `xr_main_scene` anyway: it's the documented way to request a size, and other builds may honor it.
...
- **Sizing the game window with `initScene`.** It's the start scene: only `xr_main_scene` in the manifest is read in time (and on PICO OS 6.0.0 even that was ignored; the window opened at 1280×720).
- **A page taller than 1280×720.** On PICO OS 6.0.0 windows opened at 1280×720 regardless of the requested size, and anything below the fold falls off the window. Fit lab pages in 1280×720 without scrolling.
...
5. **Why does the game window's size request go in the manifest and not `initScene`, and what size did it actually get on the emulator?**
   It's the start scene: the OS creates it from `start_url` before any of your code runs, so only `xr_main_scene` is read in time. On PICO OS 6.0.0 the request was observed not to take effect: the window opened at 1280×720, so the page is designed to fit 1280×720 without scrolling.
